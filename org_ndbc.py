...
for i in range(0,len(bnames)):
    ...
    # Read NDBC netcdf file
    fname=dpath+"/"+bnames[i][0]
    wdic = tseriesnc_ndbc(fname=fname)
    ...
    if np.any(wdic['hs']>0.1):
        ...
        wdic['hs']=quality_control_wave.landcoast_exclude(wdic,gpath='/home/ricardo/work/noaa/analysis/TC_Waves/2collocation/gridInfo_TGPM.nc',mdepth=80,mdfc=20)
        ...
        if np.size(ind)>0:
            ...
            ahs=wdic['hs'][ind]; awnd=wdic['wind_spd'][ind]
            atp=wdic['tp'][ind]; atm=wdic['tm'][ind]
            ...
            abid=np.array(np.zeros((brt.shape[0]),'i')).astype('str'); abid[:]="NDBC"+fname.split('/')[-1].split('h')[0]
...
# Save results
ind=np.where( np.nanmean(np.array([hs,wnd]),axis=0)>0.01 )
if np.size(ind)>0:
