...
if __name__ == "__main__":
    ...
    buoyd="MICROSWIFT"
    # data path
    dpath="/work/noaa/marine/ricardo.campos/work/analysis/TC_waves/data/microSWIFT"
    ...
    for i in range(0,len(bnames)):
        ...
        # Read netcdf file
        fname=dpath+"/"+bnames[i]
        try:
            wdic = tseriesnc_microswift(fname)
        except:
            ...
            if np.size(ind)>0:
                ...
                # Final arrays
                if np.size(abcid)>0:
                    aux=np.zeros(btime.shape[0],'i').astype('str'); aux[:]="MICROSWIFT"+''.join(bnames[i].split('microSWIFT')[1].split('_')[0:2])
    ...
    if np.size(ind)>0:
        ...
        df.to_csv('Data_MICROSWIFT.txt', sep='\t', index=False, header=True)
