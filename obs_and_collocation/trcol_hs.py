...
"""
Script for triple collocation (model, altimeter, in-situ obs). Hs
...
if __name__ == "__main__":
    ...
    indqq = np.where( (shs>bhs*1.3+1.) | (shs<bhs*0.75-0.8) )
    if size(indqq)>0.:
        shs[indqq[0]] = np.nan
    ...
    np.savetxt("tpresults_Hs_"+str(int(vmin)).zfill(2)+".txt", tpresults, header=header, fmt="%.6f")
