...
if __name__ == "__main__":
    ...
    buoyd="DWSD"
    # data path
    dpath="/work/noaa/marine/ricardo.campos/work/analysis/TC_waves/2collocation"
    ...
    for y in range(0,len(years)):
        ...
        fname=dpath+"/LDL_AtlanticHurricane"+year+"_4a61_8818_8454.nc"
        # LDL_AtlanticHurricane2023_4a61_8818_8454.nc
        # LDL_AtlanticHurricane2024_4a61_8818_8454.nc
        ...
        for i in range(0,len(sid)):
            ...
            if size(np.where(aux>0.2)) > 0:
                sids = np.append(sids,sid[i])

        bnames = np.array(np.array(sids).astype('int')).astype('str')
        f.close(); del f

        for i in range(0,len(bnames)):

            try:
                wdic = tseriesnc_dwsd(fname,bnames[i])

            except:
                print(" Cannot open "+fname+" "+bnames[i])
            else:

                print(" Processing QC for "+bnames[i])
                ...
                if np.size(ind)>0:
                    ...
                    # Final arrays
                    if np.size(abcid)>0:
                        aux=np.zeros(btime.shape[0],'i').astype('str'); aux[:]="DWSD"+bnames[i]
    ...
    if np.size(ind)>0:
        ...
        df.to_csv('Data_DWSD.txt', sep='\t', index=False, header=True)
