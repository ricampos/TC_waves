...
DIRO="/work/noaa/marine/ricardo.campos/data/archiveOPruns/GEFSv12Waves_AWS/netcdf/tc_waves" # final netcdf4 output path
...
if [ ! -d "$DIRW" ]; then
  echo "Directory $DIRW not found. Exiting."
  exit 1
fi
cd $DIRW
rm -f *ncks* *tmp *.nc
...
for h in $fleads;do
  for e in $ensblm;do
    ...
    if [ ${TE} -eq 1 ]; then
      ...
      wait $!
      cdo delvar,WDIR_surface,ICEC_surface ${arqn}.saux.nc ${arqn}.saux1.nc
      wait $!
      ncks -4 -L 1 -d latitude,${latmin},${latmax} ${arqn}.saux1.nc ${arqn}.saux2.nc
      wait $!
      ncks -4 -L 1 -d longitude,${lonmin},${lonmax} ${arqn}.saux2.nc ${arqn}.saux3.nc
      wait $!
      ncks -4 -L 1 ${arqn}.saux3.nc ${arqn}.saux4.nc 2>&1
      wait $!
      ncks --ppc default=.$dp ${arqn}.saux4.nc ${arqn}.nc 2>&1
      wait $!
