...
DEPENDENCIES:
 ...
 IBTRACKS V4 data. It can be downloaded at
  ...
  check if the version v04r01 is the latest
...
if __name__ == "__main__":
    ...
    ftime = np.array(np.arange(float(timegm( time.strptime('2022010100', '%Y%m%d%H') )),float(timegm( time.strptime('2026010100', '%Y%m%d%H') ))+1,3*3600.)).astype('double')
    ...
    # Cyclone data
    dfibtr = pd.read_csv('ibtracs.last4years.list.v04r01.csv', header=[0,1]) # last 4 years. I just appended 2022 to the last 3 years (2023, 24, 25)
    ...
    icyid=np.array(dfibtr['NUMBER'].values[:][:,0]).astype('int'); icseason=np.array(dfibtr['SEASON'].values[:][:,0]).astype('int')
    icyid[icseason>=(icseason.max()-3)] = icyid[icseason>=(icseason.max()-3)] + np.nanmax(icyid[icseason==icseason.min()])
    ...
    # -----------
    # Organize time
    ind=np.where((ity>=icseason.min()) & (ilat>=latm.min()) & (ilat<=latm.max()) & (ilon>=lonm.min()) & (ilon<=lonm.max()) )[0]; inat=np.copy(np.array(inat[ind]))
    ...
    print(" ")
    print(" Total of "+str(np.round(100*(ct/ftime.shape[0]),2))+"% of time in these 4 years with cyclone")
    # Total of 34.7% of time in these 4 years with cyclone
    ...
    df = pd.DataFrame(cnames, columns=['cyclone_names']) # 132 total
