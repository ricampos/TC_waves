...
PURPOSE:
 Data processing, quality control, and organizing of wave buoys (Sofar SPOTTER)
...
AUTHOR and DATE:
 09/23/2025: Ricardo M. Campos, first version.
...
# Main wave processing funtion
def wproc(wdic,bname,wdt,buoyd):

    print(" Processing QC for "+bname)
    ...
    wdic['hs']=quality_control_wave.landcoast_exclude(wdic,gpath='/work/noaa/marine/ricardo.campos/work/analysis/TC_waves/2collocation/gridInfo_TGPM.nc',mdepth=80,mdfc=5) # mdepth=20,mdfc=1
    ...
    if np.size(ind)>0:
        ...
        for t in range(0,len(at)):
            ...
            if np.size(indt)>0:
                ...
                print(" Ok data allocation "+repr(t)+" "+bname)

        del ind

        # Final arrays
        if np.size(abcid)>0:
            aux=np.zeros(btime.shape[0],'i').astype('str'); aux[:]=buoyd+bname.split('-')[-1]
    ...
    return btime,brt,abid,blat,blon,agidlat,agidlon,bhs,btp,btm,bwnd,abcmap,abcid
...
if __name__ == "__main__":
    ...
    buoyd="SPOTTER"
    # Data path
    dpath="/work/noaa/marine/ricardo.campos/work/analysis/TC_waves/data/spotter"
    ...
    for y in range(0,3):
        ...
        if year=="2024":
            ...
            hurricanes=["PRJ_HurricaneFrancine_2024","PRJ_HurricaneHelene_2024","PRJ_HurricaneMilton_2024"]

            for i in range(0,len(hurricanes)):

                with open(dpath+"/"+hurricanes[i]+"/Mission--spotter-buoys/data/Geoscience-collection--spotter-data/data/Spotter/list.txt", "r") as f:
                    files = [line.strip() for line in f if line.strip()]

                for j in range(0,len(files)):
                    fname=dpath+"/"+hurricanes[i]+"/Mission--spotter-buoys/data/Geoscience-collection--spotter-data/data/Spotter/"+files[j]

                    try:
                        wdic = tseriesnc_spotter(fname)

                    except:
                        print(" Cannot open "+fname)
                    else:
                        bname=fname.split('_')[-1].split('.nc')[0]
                        btime,brt,abid,blat,blon,agidlat,agidlon,bhs,btp,btm,bwnd,abcmap,abcid = wproc(wdic,bname,wdt,buoyd)
            ...
            fname=dpath+"/campos_hurricane_spotters_"+str(years[y])+"_spectra_with_direction_coefficients.pkl"
            with open(fname, "rb") as handle:
                data_dictionary = pickle.load(handle)

            bnames = np.array(list(data_dictionary.keys())).astype('str')
            del data_dictionary

            for i in range(0,len(bnames)):

                try:
                    wdic = tseries_spotter(fname,bnames[i].split('-')[-1])

                except:
                    print(" Cannot open "+fname+" "+bnames[i])
                else:

                    btime,brt,abid,blat,blon,agidlat,agidlon,bhs,btp,btm,bwnd,abcmap,abcid = wproc(wdic,bnames[i],wdt,buoyd)
                    ...
                    del btime,brt,abid,blat,blon,agidlat,agidlon,bhs,btp,btm,bwnd,abcmap,abcid,wdic
